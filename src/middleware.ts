...
// Next.js App Router file-convention routes that MUST bypass next-intl
// locale rewriting. Without this bypass, these paths are sent to the
// [locale] catch-all and 404 (Sprint 10 favicon deploy incident).
//
// Keep in sync with:
//   src/app/icon.tsx        -> /icon
//   src/app/apple-icon.tsx  -> /apple-icon
//   (favicon.ico is served from public/ and excluded via the matcher's
//    `.*\\..*` dot rule — no bypass needed here.)
const FILE_CONVENTION_ICON_PATHS = new Set(['/icon', '/apple-icon'])

export function middleware(request: NextRequest) {
  const { pathname } = request.nextUrl

  // ─── Next.js file-convention icons: bypass i18n entirely ───
  // These are generated by ImageResponse in src/app/{icon,apple-icon}.tsx
  // and requested by browsers as e.g. /icon?<contenthash>. The pathname
  // is plain `/icon` — next-intl would otherwise locale-rewrite it.
  if (FILE_CONVENTION_ICON_PATHS.has(pathname)) {
